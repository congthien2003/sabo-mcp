  ...
  return {
    tools: [
      ...
      },
      {
        name: "pull_workflows",
        description:
          "Pull folder .workflows từ cloud/source về folder project đã cấu hình trong env. Workflows chứa hướng dẫn cho AI agent. v1.3.0: Hỗ trợ categories (commands, workflows, skills).",
        inputSchema: {
          type: "object",
          properties: {
            targetDir: {
            ...
            category: {
              type: "string",
              enum: ["commands", "workflows", "skills"],
              description:
                "(Optional) Pull specific category. Default: commands",
            },
            allCategories: {
              type: "boolean",
              description:
                "(Optional) Pull tất cả categories (commands, workflows, skills). Mặc định: false",
            },
          },
          required: [],
        },
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  if (request.params.name === "pull_workflows") {
    const { targetDir, overwrite, filename } = request.params.arguments as any;
